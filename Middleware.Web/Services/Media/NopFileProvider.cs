...
public class NopFileProvider : PhysicalFileProvider, INopFileProvider
{
    ...
    public NopFileProvider() : base(Path.GetFullPath("/"))
    {
        WebRootPath = Path.GetFullPath("/");
    }
    ...
    protected  void DeleteDirectoryRecursive(string path)
    {
    ...
    public  string[] GetFiles(string directoryPath, string searchPattern = "", bool topDirectoryOnly = true)
    {
        ...
        return System.IO.Directory.GetFileSystemEntries(directoryPath, searchPattern,
    ...
    /// <summary>
    /// Gets a  path from a physical disk path.
    /// </summary>
    /// <param name="path">The physical disk path</param>
    /// <returns>The  path. E.g. "~/bin"</returns>
    ...
    public  string MapPath(string path)
    {
        ...
        //if virtual path has slash on the end, it should be after transform the  path to physical path too
