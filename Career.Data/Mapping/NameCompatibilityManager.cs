...
public static partial class NameCompatibilityManager
{
    ...
    private static void Initialize()
    {
        ...
        lock (_initLock)
        {
            ...
            {
                var providerType = nameCompatibility.GetType();
                if (_loadedFor.Contains(providerType))
                    continue;

                _loadedFor.Add(providerType);

                // Use Contains check only inside the lock to avoid race
                foreach (var (key, value) in nameCompatibility.TableNames.Where(tableName => !_tableNames.ContainsKey(tableName.Key)))
                    _tableNames.Add(key, value);

                foreach (var (key, value) in nameCompatibility.ColumnName.Where(columnName => !_columnName.ContainsKey(columnName.Key)))
