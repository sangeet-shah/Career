...
public class PictureService : IPictureService
{
    ...
    {
        // create generic atttribute to Banner Physical store mapping list
        var bannerPhysicalStoreMapping = await _genericAttributeRepository.Table.Where(x => x.KeyGroup == nameof(Banner) && x.Key == bannerPhysicalStoreMap).ToListAsync();
        ...
        var banners = (from b in _bannerRepository.Table
                       where b.DisplaySection == 40 && b.Enabled == true &&
                       (b.StartDate <= dateTimeNow || b.StartDate == null) && (dateTimeNow <= b.EndDate || b.EndDate == null)
                       select b).ToList();
